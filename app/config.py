...
from app import configs as fatsiagrowth_configs
...
def save_config(config):
    # Local config file
    user_config_file = osp.join(osp.expanduser("~"), ".mowbotapprc")
    try:
...
def get_default_config():
    config_file = "mowbot_app.yaml"
    with pkg_resources.open_text(fatsiagrowth_configs, config_file) as f:
        config = yaml.safe_load(f)

    # Save default config to ~/.mowbotapprc
    if not osp.exists(osp.join(osp.expanduser("~"), ".mowbotapprc")):
        save_config(config)
